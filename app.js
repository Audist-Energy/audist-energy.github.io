...
const REPORT_RENDERER = globalThis.AudistReportRenderer;
const FIELD_WORKFLOW = globalThis.AudistFieldWorkflow;
const SITE_EVIDENCE = globalThis.AudistSiteEvidence;
...
async function openLevel2Report(reportId){const report=(currentAudit.reports||[]).find(r=>r.reportId===reportId);if(!report)return;const professional=report.reportSchemaVersion===REPORT_RENDERER?.MODEL_VERSION,html=professional&&globalThis.AudistCanonicalReport?AudistCanonicalReport.renderHtml(AudistCanonicalReport.buildTemplateData(currentAudit,{preparedBy:report.preparedBy,status:report.status,statusTag:report.status})):professional?REPORT_RENDERER.renderHtml(report,currentAudit,{photoSources:await reportPhotoSources(report)}):REPORT_ENGINE.renderHtml(report,currentAudit),url=URL.createObjectURL(new Blob([html],{type:"text/html"}));window.open(url,"_blank","noopener");setTimeout(()=>URL.revokeObjectURL(url),300000);}
