...
const CALC_ENGINE = globalThis.FalconProCalculations;
const WORKFLOW = globalThis.FalconProWorkflow;
const PACKAGE_EXPORT = globalThis.FalconProPackageExport;
const UTILITY_ANALYSIS = globalThis.FalconProUtilityAnalysis;
const END_USE_ANALYSIS = globalThis.FalconProEndUseAnalysis;
const PORTFOLIO_ANALYSIS = globalThis.FalconProPortfolioAnalysis;
const ADVANCED_ANALYSIS = globalThis.FalconProAdvancedAnalysis;
const QA_ENGINE = globalThis.FalconProQaRules;
const AI_REVIEW = globalThis.FalconProAiReview;
const REPORT_ENGINE = globalThis.FalconProReportEngine;
const REPORT_RENDERER = globalThis.FalconProReportRenderer;
const FIELD_WORKFLOW = globalThis.FalconProFieldWorkflow;
const SITE_EVIDENCE = globalThis.FalconProSiteEvidence;
...
const MEASUREMENT_PRESETS = {
  PackagedHVAC:[["Outside air temperature","°F"],["Return air temperature","°F"],["Mixed air temperature","°F"],["Supply air temperature","°F"],["Economizer damper position","%"],["Supply RH","% RH"],["Return RH","% RH"],["Airflow","cfm"],["Static pressure","in. w.c."],["Fan amps","A"],["Fan power","kW"],["VFD speed","Hz"]],
  AirHandling:[["Outside air temperature","°F"],["Return air temperature","°F"],["Mixed air temperature","°F"],["Supply air temperature","°F"],["Economizer damper position","%"],["Supply RH","% RH"],["Return RH","% RH"],["Airflow","cfm"],["Static pressure","in. w.c."],["Fan amps","A"],["Fan power","kW"],["VFD speed","Hz"]],
  ...
  Lighting:[["Light level","fc"],["Measured fixture watts","W"],["Lighting controls status","state"]],
...
function blankAudit(){
  return {
    ...
    utilityAccounts:[],
    auditScope:{includedAreas:[],excludedAreas:[],includedSystems:[],excludedSystems:[],limitations:[],inaccessibleAreas:[],unavailableData:[],operatingConditionDuringVisit:"",basisNotes:"",auditStandard:"",auditLevel:""},
    facilitySchedule:{occupiedHours:"",daySchedules:[],seasonalNotes:"",shutdownPeriods:[]},
    auditStandardStatus:"NOT_SET",siteVisits:[],peopleInterviewed:[],observations:[],incentives:[],benchmarkAnalysis:null,mvCapability:null,portfolioTierRules:null,
    systems:[],
    equipment:[],
    ecms:[],
    ...
    metadata:{app:"Falcon Pro", appVersion:APP_VERSION, storage:"IndexedDB", intendedStandard:"ASHRAE Level 2 support"}
  };
}
...
function migrateAudit(audit){
  ...
  if(oldVersion < 3){
    ...
    migrated.metadata = {...(migrated.metadata||{}), app:"Falcon Pro", appVersion:APP_VERSION};
  }
...
async function saveCurrent(){
  ...
  try{
    ...
    currentAudit.metadata={...(currentAudit.metadata||{}),app:"Falcon Pro",appVersion:APP_VERSION};
    recalculateAllCompleteness();
...
function siteEvidenceInputChanged(){
  currentAudit.facilitySchedule=currentAudit.facilitySchedule||{occupiedHours:"",daySchedules:[],seasonalNotes:"",shutdownPeriods:[]};currentAudit.auditScope=currentAudit.auditScope||{limitations:[],inaccessibleAreas:[],unavailableData:[]};currentAudit.siteVisits=Array.isArray(currentAudit.siteVisits)?currentAudit.siteVisits:[];
  currentAudit.facilitySchedule.occupiedHours=$("facility-occupied-hours").value;currentAudit.facilitySchedule.seasonalNotes=$("facility-seasonal-notes").value;currentAudit.auditScope.operatingConditionDuringVisit=$("scope-operating-condition").value;currentAudit.auditScope.limitations=$("scope-limitations").value.split("\n").map(x=>x.trim()).filter(Boolean);
  const date=$("site-visit-date").value;if(date){const visit=currentAudit.siteVisits[0]||{visitId:uid(),startTime:"",endTime:"",operatingCondition:"",notes:""};visit.date=date;visit.purpose=$("site-visit-purpose").value;visit.outdoorConditions=$("site-visit-outdoor").value;currentAudit.siteVisits[0]=visit;}queueSave();
}
...
async function systemScopeChanged(e){
  ...
  }else if(["intervalMeterAvailable","dedicatedSubmeterAvailable","temporaryMeteringFeasible","basTrendingAvailable","controlPointsAccessible"].includes(key)){
    draftEquipment.mvCapability=draftEquipment.mvCapability||{};draftEquipment.mvCapability[key]=e.target.value;
  }else if(["basPresent","basPlatform","basAccessible","trendAvailable","trendExportAvailable","scheduleEditable","setpointsEditable","trendInterval","trendRetention"].includes(key)){
    draftEquipment.basCapability=draftEquipment.basCapability||{};const target={basPlatform:"platform",basAccessible:"accessible"}[key]||key;draftEquipment.basCapability[target]=e.target.value;const system=currentAudit.systems.find(s=>s.systemRecordId===draftEquipment.systemRecordId);if(system)system.basCapability=structuredClone(draftEquipment.basCapability);
  }else{
...
function renderEquipmentContext(){
  ...
  const observations=(currentAudit.observations||[]).filter(o=>(o.equipmentRecordIds||[]).includes(draftEquipment.recordId));
  $("equipment-observation-list").innerHTML=observations.map(o=>`<div class="item"><strong>${escapeHtml(o.severity)} — ${escapeHtml(o.title)}</strong><small>${escapeHtml(o.type.replaceAll("_"," "))} • ${escapeHtml(o.description)}</small></div>`).join("")||`<p class="muted">No structured observations.</p>`;
}

async function addEquipmentObservation(){
  if(!draftEquipment||!SITE_EVIDENCE)return;const type=prompt(`Observation type:\n${SITE_EVIDENCE.OBSERVATION_TYPES.join("\n")}`,"PHYSICAL_CONDITION");if(!type)return;const severity=prompt(`Severity:\n${SITE_EVIDENCE.SEVERITIES.join("\n")}`,"MODERATE");if(!severity)return;const description=prompt("Short observation / deficiency description","");if(!description?.trim())return;let observation;try{observation=SITE_EVIDENCE.createObservation({type:type.trim().toUpperCase(),severity:severity.trim().toUpperCase(),description,equipmentRecordIds:[draftEquipment.recordId],systemRecordIds:[draftEquipment.systemRecordId],photoIds:[],opportunityFlagIds:[],provenance:"Observed"},{observationId:uid(),now:nowISO()});}catch(error){alert(error.message);return;}currentAudit.observations=currentAudit.observations||[];currentAudit.observations.push(observation);if(await saveCurrent())renderEquipmentContext();else currentAudit.observations=currentAudit.observations.filter(o=>o!==observation);
}

function renderEquipmentFields(type, values={}){
  const groups={core:[],recommended:[],controls:[],advanced:[]};
  const evidenceFields=[["servedArea","Area Served","Sales Floor / Pharmacy / Entire Building"],["servedZone","Served Zone","North Zone"],["serviceDescription","Service Relationship","Serves sales floor and pharmacy"],["approximateAreaServedSqFt","Approx. Area Served","ft²"],["equipmentScheduleType","Schedule Relationship",""],["condition","Condition",""],["installYear","Install Year",""],["assessmentBasis","Condition Assessment Basis","Observed condition / records"],["conditionNote","Condition Note",""],["controlType","Control Type",""],["controlLocation","Control Location",""],["sequenceSummary","Sequence Summary",""],["afterHoursBehavior","After-Hours Behavior",""],["seasonalControlNotes","Seasonal Control Notes",""],["intervalMeterAvailable","Interval Meter Available",""],["dedicatedSubmeterAvailable","Dedicated Submeter Available",""],["temporaryMeteringFeasible","Temporary Metering Feasible",""],["basTrendingAvailable","BAS Trending Available",""],["controlPointsAccessible","Control Points Accessible",""],...(type==="BASControls"?[["basPresent","BAS Present",""],["basPlatform","Platform",""],["basAccessible","Accessible",""],["trendAvailable","Trend Available",""],["trendExportAvailable","Trend Export Available",""],["scheduleEditable","Schedule Editable",""],["setpointsEditable","Setpoints Editable",""],["trendInterval","Trend Interval",""],["trendRetention","Trend Retention",""]]:[])];
  const fields=[...(schemas[type]||schemas.Other),...evidenceFields,...(type==="Refrigeration"&&FIELD_WORKFLOW?FIELD_WORKFLOW.refrigerationFields(values.equipmentSubtype).map(([key,label])=>[key,label,""]):[])];
  const displayValues={...values,...(values.mvCapability||{}),basPresent:values.basCapability?.basPresent||"UNKNOWN",basPlatform:values.basCapability?.platform||"",basAccessible:values.basCapability?.accessible||"UNKNOWN",trendAvailable:values.basCapability?.trendAvailable||"UNKNOWN",trendExportAvailable:values.basCapability?.trendExportAvailable||"UNKNOWN",scheduleEditable:values.basCapability?.scheduleEditable||"UNKNOWN",setpointsEditable:values.basCapability?.setpointsEditable||"UNKNOWN",trendInterval:values.basCapability?.trendInterval||"",trendRetention:values.basCapability?.trendRetention||"",condition:values.conditionAssessment?.condition||"NOT_ASSESSED",installYear:values.conditionAssessment?.installYear||"",assessmentBasis:values.conditionAssessment?.assessmentBasis||"",conditionNote:values.conditionAssessment?.note||""};
  ...
  const fieldHtml=([id,label,ph])=>id==="equipmentSubtype"
    ...
    : id==="equipmentScheduleType"?`<label>${label}<select data-equipment-field="${id}" id="f_${id}">${(SITE_EVIDENCE?.SCHEDULE_TYPES||["SAME_AS_FACILITY","CONTINUOUS","CUSTOM","UNKNOWN"]).map(v=>`<option ${displayValues[id]===v?"selected":""}>${v}</option>`).join("")}</select></label>`
    : id==="condition"?`<label>${label}<select data-equipment-field="${id}" id="f_${id}">${(SITE_EVIDENCE?.CONDITIONS||["GOOD","FAIR","POOR","FAILED","NOT_ASSESSED"]).map(v=>`<option ${displayValues[id]===v?"selected":""}>${v}</option>`).join("")}</select></label>`
    : ["basPresent","basAccessible","trendAvailable","trendExportAvailable","scheduleEditable","setpointsEditable","intervalMeterAvailable","dedicatedSubmeterAvailable","temporaryMeteringFeasible","basTrendingAvailable","controlPointsAccessible"].includes(id)?`<label>${label}<select data-equipment-field="${id}" id="f_${id}">${(SITE_EVIDENCE?.STATES||["PRESENT","NOT_PRESENT","UNKNOWN","NOT_INSPECTED","NOT_APPLICABLE"]).map(v=>`<option ${displayValues[id]===v?"selected":""}>${v}</option>`).join("")}</select></label>`
    : `<label>${label}<input data-equipment-field="${id}" id="f_${id}" placeholder="${ph}" value="${escapeHtml(displayValues[id]||"")}">${id==="equipmentId"?"":`<select data-equipment-provenance="${id}" aria-label="${escapeHtml(label)} provenance"><option value="">Provenance...</option>${["Measured","Nameplate","Estimated","Assumed","Calculated"].map(p=>`<option ${values.fieldProvenance?.[id]===p?"selected":""}>${p}</option>`).join("")}</select>`}</label>`;
...
function equipmentFieldChanged(e){
  ...
  if(["condition","installYear","assessmentBasis","conditionNote"].includes(key)){
    draftEquipment.conditionAssessment=draftEquipment.conditionAssessment||{condition:"NOT_ASSESSED",installYear:"",approximateAgeYears:null,assessmentBasis:"",note:"",assessedAt:null};
    const target={condition:"condition",installYear:"installYear",assessmentBasis:"assessmentBasis",conditionNote:"note"}[key];draftEquipment.conditionAssessment[target]=e.target.value;
    draftEquipment.conditionAssessment.assessedAt=nowISO();
  }else{
    draftEquipment[key]=e.target.value;
    if(key==="equipmentScheduleType"&&e.target.value==="CUSTOM")draftEquipment.equipmentSchedule={summary:draftEquipment.schedule||"",provenance:"Field entry"};
  }
...
function renderReportConfiguration(){if(!REPORT_RENDERER||$("report-section-config").dataset.ready)return;const rows=[...REPORT_RENDERER.DEFAULT_SECTIONS.map(s=>({kind:"section",id:s.sectionId,label:s.heading,order:s.order})),...REPORT_RENDERER.DEFAULT_APPENDICES.map(a=>({kind:"appendix",id:a.appendixId,label:a.title,order:a.order}))];$("report-section-config").innerHTML=rows.map(r=>`<label class="scope-option"><input type="checkbox" checked data-report-${r.kind}="${r.id}"> ${escapeHtml(r.label)} <input class="small-order" type="number" min="0" value="${r.order}" data-report-order="${r.kind}:${r.id}" aria-label="${escapeHtml(r.label)} order"></label>`).join("");$("report-section-config").dataset.ready="1";}
function reportStructureOptions(){const sections=REPORT_RENDERER.DEFAULT_SECTIONS.map(s=>({...s,include:Boolean(document.querySelector(`[data-report-section="${s.sectionId}"]`)?.checked),order:Number(document.querySelector(`[data-report-order="section:${s.sectionId}"]`)?.value??s.order)})),appendices=REPORT_RENDERER.DEFAULT_APPENDICES.map(a=>({...a,include:Boolean(document.querySelector(`[data-report-appendix="${a.appendixId}"]`)?.checked),order:Number(document.querySelector(`[data-report-order="appendix:${a.appendixId}"]`)?.value??a.order)}));return {sections,appendices};}
async function generateProfessionalReport(){if(!REPORT_RENDERER){alert("The professional renderer is unavailable. Refresh once online and retry.");return;}await flushPendingSave();const prior=structuredClone(currentAudit.reports||[]),theme={companyName:$("professional-company").value.trim()||"Falcon Pro",preparedBy:$("professional-prepared-by").value.trim(),accentColor:$("professional-accent").value,footerText:$("professional-footer").value.trim(),confidentialityText:$("professional-confidentiality").value.trim(),logo:new URL("falcon-pro-icon.png?v=falcon-pro-1",location.href).href};try{const existing=(currentAudit.reports||[]).filter(r=>r.reportSchemaVersion===REPORT_RENDERER.MODEL_VERSION).at(-1),model=REPORT_RENDERER.buildModel(currentAudit,{reportId:uid(),generatedAt:nowISO(),reportTitle:$("professional-report-title").value.trim(),preparedFor:$("professional-prepared-for").value.trim(),preparedBy:theme.preparedBy,projectNumber:$("professional-project-number").value.trim(),theme,...reportStructureOptions()});if(existing){existing.status="SUPERSEDED";model.revision=(existing.revision||0)+1;model.revisions=[...(existing.revisions||[]),{revision:model.revision,date:nowISO().slice(0,10),note:"Refreshed deterministic report",sourceAuditUpdatedAt:currentAudit.updatedAt||null}];}currentAudit.reports=[...(currentAudit.reports||[]),model];if(await saveCurrent()){render();await openLevel2Report(model.reportId);}else currentAudit.reports=prior;}catch(error){currentAudit.reports=prior;alert(`Report generation failed.\n\n${error.message||error}`);}}
...
async function reportPhotoSources(report){const out={};for(const figure of report.figures||[]){const equipment=(currentAudit.equipment||[]).find(e=>(e.photos||[]).some(p=>p.photoId===figure.photoId)),photo=equipment?.photos?.find(p=>p.photoId===figure.photoId);if(photo?.dataUrl){out[figure.photoId]=photo.dataUrl;continue;}const stored=await dbGetPhoto(figure.photoId);if(stored?.blob)out[figure.photoId]=await blobDataUrl(stored.blob);}return out;}
async function openLevel2Report(reportId){const report=(currentAudit.reports||[]).find(r=>r.reportId===reportId);if(!report)return;const professional=report.reportSchemaVersion===REPORT_RENDERER?.MODEL_VERSION,html=professional&&globalThis.FalconProCanonicalReport?FalconProCanonicalReport.renderHtml(FalconProCanonicalReport.buildTemplateData(currentAudit,{preparedBy:report.preparedBy,status:report.status,statusTag:report.status})):professional?REPORT_RENDERER.renderHtml(report,currentAudit,{photoSources:await reportPhotoSources(report)}):REPORT_ENGINE.renderHtml(report,currentAudit),url=URL.createObjectURL(new Blob([html],{type:"text/html"}));window.open(url,"_blank","noopener");setTimeout(()=>URL.revokeObjectURL(url),300000);}
function editLevel2Section(reportId,sectionId){const index=(currentAudit.reports||[]).findIndex(r=>r.reportId===reportId),report=currentAudit.reports?.[index],section=report?.sections?.find(s=>s.sectionId===sectionId);if(!section)return;const professional=report.reportSchemaVersion===REPORT_RENDERER?.MODEL_VERSION,content=prompt(`Edit ${section.heading}. This changes report narrative only.`,professional?section.narrative:section.content);if(content===null)return;const prior=report;currentAudit.reports[index]=professional?REPORT_RENDERER.editNarrative(report,sectionId,content,nowISO()):REPORT_ENGINE.editSection(report,sectionId,content,nowISO());saveCurrent().then(ok=>{if(!ok)currentAudit.reports[index]=prior;render();});}
async function markReportReady(reportId){const index=(currentAudit.reports||[]).findIndex(r=>r.reportId===reportId),report=currentAudit.reports[index];if(report?.reportSchemaVersion!==REPORT_RENDERER?.MODEL_VERSION)return;if(REPORT_RENDERER.isStale(report,currentAudit)){alert("This report is stale. Regenerate it before engineer approval or issue.");return;}const check=REPORT_RENDERER.integrity(report,currentAudit);if(check.status==="FAIL"){alert(check.errors.join("\n"));return;}if(!confirm("Confirm engineer review and mark this report READY TO ISSUE? This does not guarantee savings or alter engineering records."))return;const prior=report;currentAudit.reports[index]=REPORT_RENDERER.revise(report,currentAudit,{status:"READY_TO_ISSUE",note:"Engineer review complete",at:nowISO()});if(await saveCurrent())render();else currentAudit.reports[index]=prior;}
function renderLevel2Reports(){if(!REPORT_ENGINE)return;renderReportConfiguration();const gate=REPORT_ENGINE.readiness(currentAudit),reports=currentAudit.reports||[],latest=reports.at(-1),professional=latest?.reportSchemaVersion===REPORT_RENDERER?.MODEL_VERSION,integrity=latest?(professional?REPORT_RENDERER.integrity(latest,currentAudit):REPORT_ENGINE.integrity(latest,currentAudit)):null,stale=latest?(professional?REPORT_RENDERER.isStale(latest,currentAudit):REPORT_ENGINE.isStale(latest,currentAudit)):false;$("report-readiness-summary").innerHTML=`<div class="metric"><strong>${escapeHtml(currentAuditQa()?.readiness?.replaceAll("_"," ")||"UNAVAILABLE")}</strong><span>Engineering QA</span></div><div class="metric"><strong>${escapeHtml(gate.status.replaceAll("_"," "))}</strong><span>Report Source</span></div><div class="metric"><strong>${latest?escapeHtml(stale?"STALE":latest.status||"CURRENT"):"NO DRAFT"}</strong><span>Draft Report</span></div><div class="metric"><strong>${escapeHtml(integrity?.status?.replaceAll("_"," ")||"NOT CHECKED")}</strong><span>Integrity</span></div>`;$("report-drafts").innerHTML=reports.slice().reverse().map(r=>{const isProfessional=r.reportSchemaVersion===REPORT_RENDERER?.MODEL_VERSION,check=isProfessional?REPORT_RENDERER.integrity(r,currentAudit):REPORT_ENGINE.integrity(r,currentAudit),isReportStale=isProfessional?REPORT_RENDERER.isStale(r,currentAudit):REPORT_ENGINE.isStale(r,currentAudit);return `<details class="disclosure"><summary><span>${escapeHtml(r.reportTitle)}</span><span class="pill">${isReportStale?"STALE":escapeHtml(r.status||check.status)}</span></summary>${isReportStale?`<p class="badge-warn">REPORT STALE — underlying engineering data changed</p>`:""}<p>Revision ${r.revision??0} • ${r.sections.length} sections • ${r.figures.length} selected figures • ${r.limitations.length} limitations • Integrity ${escapeHtml(check.status)}</p><div class="actions"><button class="secondary small" onclick="openLevel2Report('${r.reportId}')">Preview Report</button>${isProfessional?`<button class="small" onclick="openLevel2Report('${r.reportId}')">Print / Save PDF</button><button class="secondary small" onclick="markReportReady('${r.reportId}')">Mark Ready to Issue</button>`:""}</div>${r.sections.filter(s=>s.include!==false).map(s=>`<button class="secondary small" onclick="editLevel2Section('${r.reportId}','${s.sectionId}')">Edit ${escapeHtml(s.heading)}</button>`).join(" ")}</details>`;}).join("")||`<p class="muted">No report draft exists. Generate a deterministic professional report when the engineering record is ready.</p>`;}
...
function renderFieldExitReviewV63(){if(!SITE_EVIDENCE)return;const review=SITE_EVIDENCE.fieldExitReview(currentAudit,FIELD_WORKFLOW?.fieldExitOpportunityItems(currentAudit)||[]),icon={BLOCKING:"🔴",IMPORTANT:"🟡",OPTIONAL:"○"};$("field-exit-count").textContent=`${review.items.length} item${review.items.length===1?"":"s"}`;$("field-exit-status").innerHTML=review.status==="READY_TO_LEAVE_SITE"?`<p class="badge-ok"><strong>READY TO LEAVE SITE</strong></p>`:`<p class="badge-warn"><strong>${review.blocking?`${review.blocking} BLOCKING ITEM${review.blocking===1?"":"S"}`:`${review.important} IMPORTANT ITEM${review.important===1?"":"S"}`} NEED ATTENTION</strong></p>`;$("field-exit-review").innerHTML=review.items.length?review.items.map(item=>`<div class="item field-exit-item"><strong>${icon[item.severity]} ${escapeHtml(item.title)}</strong><small>${escapeHtml(item.category)} • ${escapeHtml(item.label)}</small></div>`).join(""):`<p class="badge-ok">✓ Difficult-to-recover onsite evidence is complete for the active context.</p>`;}
...
function render(){
  ...
  $("header-status").textContent=`${currentAudit.site?.facilityName||"Active audit"} • V7.0 Prototype`;
  currentAudit.facilitySchedule=currentAudit.facilitySchedule||{occupiedHours:"",daySchedules:[],seasonalNotes:"",shutdownPeriods:[]};currentAudit.auditScope=currentAudit.auditScope||{limitations:[]};currentAudit.siteVisits=currentAudit.siteVisits||[];$("facility-occupied-hours").value=currentAudit.facilitySchedule.occupiedHours||"";$("facility-seasonal-notes").value=currentAudit.facilitySchedule.seasonalNotes||"";$("scope-operating-condition").value=currentAudit.auditScope.operatingConditionDuringVisit||"";$("scope-limitations").value=(currentAudit.auditScope.limitations||[]).join("\n");$("site-visit-date").value=currentAudit.siteVisits[0]?.date||"";$("site-visit-purpose").value=currentAudit.siteVisits[0]?.purpose||"";$("site-visit-outdoor").value=currentAudit.siteVisits[0]?.outdoorConditions||"";
...
async function saveOrSharePackage(){if(!preparedPackage)return;const file=new File([preparedPackage.blob],preparedPackage.fileName,{type:"application/zip"});try{if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({files:[file],title:"Falcon Pro Audit Package"});return;}}catch(error){if(error?.name==="AbortError")return;console.warn(error);}downloadPackage();}
...
for(const id of ["facility-occupied-hours","facility-seasonal-notes","scope-operating-condition","scope-limitations","site-visit-date","site-visit-purpose","site-visit-outdoor"])$(id).addEventListener("input",siteEvidenceInputChanged);
$("add-observation-btn").onclick=addEquipmentObservation;
...
$("generate-professional-report-btn").onclick=generateProfessionalReport;
...
showDashboard();
if("serviceWorker" in navigator){ navigator.serviceWorker.register("sw.js?v=7.0.0").catch(console.error); }
